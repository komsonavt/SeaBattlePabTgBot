...
object ForumWorkspace {
    fun ensure(client: TelegramClient, config: BotConfig, store: CommunityStore): ForumTopics {
        ...
        fun topic(key: String, title: String, fallback: Int?): Int {
            ...
            val created: ForumTopic = client.execute(CreateForumTopic(config.moderationChatId.toString(), title))
            return created.messageThreadId.also { store.saveForumTopic(key, it); createdKeys.add(key) }
        }
        return ForumTopics(
            topic("moderation", "🛟 Заявки в игру", config.moderationTopicId),
            topic("exports", "📊 Выгрузки и статистика", config.exportsTopicId),
            topic("broadcasts", "📣 Рассылки", config.broadcastsTopicId), createdKeys
        )
    }
}
